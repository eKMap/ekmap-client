...
export default function (olLayer, glStyle, source, resolutions, spriteData, spriteImageUrl, getFonts) {
    ...
    var styleFunction = function (feature, resolution) {
        ...
        var layers = layersBySourceLayer["1902"];
        if (!layers) {
            return;
        }
        ...
        var _loop_1 = function (i, ii) {
            ...
            if (!filter || evaluateFilter(layerId, filter, f, zoom)) {
                ...
                var color = void 0, opacity = void 0, fill = void 0, stroke = void 0, strokeColor = void 0;
                ...
                if (type != 1 && layer.type == 'line') {
                    ...
                    if (color && width_1 > 0) {
                        ...
                        if (!style) {
                            style = {
                                lineCap: getValue(layer, 'layout', 'line-cap', zoom, f),
                                lineJoin: getValue(layer, 'layout', 'line-join', zoom, f),
                                color: color,
                                fillOpacity: 1,
                                weight: width_1
                            }
                        }else{
                            style.lineCap = getValue(layer, 'layout', 'line-cap', zoom, f);
                            style.lineJoin = getValue(layer, 'layout', 'line-join', zoom, f);
                            style.color = color;
                            style.fillOpacity = 1;
                            style.weight = width_1;
                        }
                        // stroke = style.getStroke();
                ...
                if (type == 1 && 'circle-radius' in paint) {
                    ...
                    if(!style){
                        style = {
                            radius: circleRadius,
                            stroke: circleStrokeColor && circleStrokeWidth > 0 ? true : false,
                            fillColor: circleColor ? circleColor : undefined
                        }
                    }else{
                        style.fillColor = circleColor ? circleColor : undefined;
                        style.radius = circleRadius;
                        style.stroke = circleStrokeColor && circleStrokeWidth > 0 ? true : false;
                    }
