 ...
 * @return {mapboxgl.ekmap.control.Geocoder} `this`
 *
 */
export class Geocoder extends mapboxgl.Evented {
    constructor(options) {
        ...
        this.options.flyTo = true;
        this.options.trackProximity = true;
        this.options.minLength = 2;
        this.options.reverseGeocode = false;
        this.options.limit = 5;
        this.options.origin = 'https://api.mapbox.com';
        this.options.enableEventLogging = true;
        this.options.marker = true;
        this.options.mapboxgl = mapboxgl;
        this.options.collapsed = false;
        this.options.clearAndBlurOnEsc = false;
        this.options.clearOnBlur = false;
        this.target = this.options.target;
        this.options.getItemValue = function(item) {
    ...
    onAdd(map) {
        ...
        if (this.options.collapsed) {
            this._collapse();
            if (this.target) {
                this.targetContainer.addEventListener('mouseenter', this._unCollapse);
                this.targetContainer.addEventListener('mouseleave', this._collapse);
            } else {
    ...
    _onChange() {
        ...
        if (selected && JSON.stringify(selected) !== this.lastSelected) {
            ...
            /**
             * @event mapboxgl.ekmap.control.Geocoder#result
             * @description Fired when the feature is selected.
             */
     ...
     * @returns {mapboxgl.ekmap.control.Geocoder} this
     */
    query(searchInput) {
     ...
     * @returns {mapboxgl.ekmap.control.Geocoder}
     */
    setLimit(limit) {
