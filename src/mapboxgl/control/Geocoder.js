...
export class Geocoder extends mapboxgl.Evented {
    ...
    _geocode(searchInput) {
        ...
        // make the request a GeocoderGeocode
        else if (
            this.options.GeocoderGeocode &&
            ...
            // client only accepts one type for GeocoderGeocode, so
            // use first config type if one, if not default to poi
            config.types ? [config.types[0]] : ["poi"];
            config = extend(config, { query: coords, limit: 1 });

            // drop proximity which may have been set by trackProximity since it's not supported by the GeocoderGeocoder
            if ('proximity' in config) {
                delete config.proximity;
            }

            request = this.geocoderService.GeocoderGeocode(config).send();
        } else {
