export class Eventful {
    constructor() {
        /**
         * @member {Object} Ekmap.LevelRenderer.Eventful.prototype._handlers
         * @description event processing object (event dispatcher).
         */
        this._handlers = {};

        this.CLASS_NAME = "Ekmap.LevelRenderer.Eventful";
    }


    /**
     * @function {Object} Ekmap.LevelRenderer.Eventful.prototype.destroy
     * @description destroys the object and releases resources. All properties will be set to null after calling this function.
     */
    destroy() {
        this._handlers = null;
    }


    /**
     * @function Ekmap.LevelRenderer.Eventful.prototype.one
     * @description Single trigger binding, destroy after dispatch.
     * @param {string} event-event name.
     * @param {boolean} handler-response function.
     * @param {Object} context-context.
     * @returns {Ekmap.LevelRenderer.Eventful} this
     */
    one(event, handler, context) {
        var _h = this._handlers;

        if (!handler || !event) {
            return this;
        }

        if (!_h[event]) {
            _h[event] = [];
        }

        _h[event].push({
            h: handler,
            one: true,
            ctx: context || this
        });

        return this;
    }


    /**
     * @function Ekmap.LevelRenderer.Eventful.prototype.bind
     * @description Binding event.
     * @param {string} event-event name.
     * @param {boolean} handler-response function.
     * @param {Object} context-context.
     * @returns {Ekmap.LevelRenderer.Eventful} this
     */
    bind(event, handler, context) {
        var _h = this._handlers;

        if (!handler || !event) {
            return this;
        }

        if (!_h[event]) {
            _h[event] = [];
        }

        _h[event].push({
            h: handler,
            one: false,
            ctx: context || this
        });

        return this;
    }


    /**
     * @function Ekmap.LevelRenderer.Eventful.prototype.unbind
     * @description unbind event.
     * @param {string} event-event name.
     * @param {boolean} handler-response function.
     * @returns {Ekmap.LevelRenderer.Eventful} this
     */
    unbind(event, handler) {
        var _h = this._handlers;

        if (!event) {
            this._handlers = {};
            return this;
        }

        if (handler) {
            if (_h[event]) {
                var newList = [];
                for (var i = 0, l = _h[event].length; i <l; i++) {
                    if (_h[event][i]['h'] != handler) {
                        newList.push(_h[event][i]);
                    }
                }
                _h[event] = newList;
            }

            if (_h[event] && _h[event].length === 0) {
                delete _h[event];
            }
        } else {
            delete _h[event];
        }

        return this;
    }


    /**
     * @function Ekmap.LevelRenderer.Eventful.prototype.dispatch
     * @description event distribution.
     * @param {string} type-Event type.
     * @returns {Ekmap.LevelRenderer.Eventful} this
     */
    dispatch(type) {
        if (this._handlers[type]) {
            var args = arguments;
            var argLen = args.length;

            if (argLen> 3) {
                args = Array.prototype.slice.call(args, 1);
            }

            var _h = this._handlers[type];
            var len = _h.length;
            for (var i = 0; i <len;) {
                // Optimize advise from backbone
                switch (argLen) {
                    case 1:
                        _h[i]['h'].call(_h[i]['ctx']);
                        break;
                    case 2:
                        _h[i]['h'].call(_h[i]['ctx'], args[1]);
                        break;
                    case 3:
                        _h[i]['h'].call(_h[i]['ctx'], args[1], args[2]);
                        break;
                    default:
                        // have more than 2 given arguments
                        _h[i]['h'].apply(_h[i]['ctx'], args);
                        break;
                }

                if (_h[i]['one']) {
                    _h.splice(i, 1);
                    len--;
                } else {
                    i++;
                }
            }
        }

        return this;
    }


    /**
     * @function Ekmap.LevelRenderer.Eventful.prototype.dispatchWithContext
     * @description Event distribution with context, the last parameter is the context of the event callback.
     * @param {string} type-Event type.
     * @returns {Ekmap.LevelRenderer.Eventful} this
     */
    dispatchWithContext(type) {
        if (this._handlers[type]) {
            var args = arguments;
            var argLen = args.length;

            if (argLen> 4) {
                args = Array.prototype.slice.call(args, 1, args.length-1);
            }
            var ctx = args[args.length-1];

            var _h = this._handlers[type];
            var len = _h.length;
            for (var i = 0; i <len;) {
                // Optimize advise from backbone
                switch (argLen) {
                    case 1:
                        _h[i]['h'].call(ctx);
                        break;
                    case 2:
                        _h[i]['h'].call(ctx, args[1]);break;
                        case 3:
                            _h[i]['h'].call(ctx, args[1], args[2]);
                            break;
                        default:
                            // have more than 2 given arguments
                            _h[i]['h'].apply(ctx, args);
                            break;
                    }
   
                    if (_h[i]['one']) {
                        _h.splice(i, 1);
                        len--;
                    } else {
                        i++;
                    }
                }
            }
   
            return this;
        }

}