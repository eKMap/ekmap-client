...
/**
 * @class Ekmap.DensityKernelAnalystParameters
 ...
 * @param {(Ekmap.Bounds|L.Bounds|ol.extent)} [options.bounds] - 核密度分析的范围，用于确定结果栅格数据集的范围。如果缺省，则默认为原数据集的范围。 
...
export class DensityKernelAnalystParameters {
    ...
    constructor(options) {
        /**
         * @member {string} Ekmap.DensityKernelAnalystParameters.prototype.dataset
        ...
        /**
         * @member {(Ekmap.Bounds|L.Bounds|ol.extent)} [Ekmap.DensityKernelAnalystParameters.prototype.bounds]
        ...
        /**
         * @member {string} Ekmap.DensityKernelAnalystParameters.prototype.fieldName
         * @description 用于进行核密度分析的测量值的字段名称，核密度分析不支持文本类型的字段。
         */
        this.fieldName = null;

        /**
         * @member {number} [Ekmap.DensityKernelAnalystParameters.prototype.resultGridDatasetResolution]
         * @description 密度分析结果栅格数据的分辨率，单位与当前数据集相同。默认值为当前数据集的长宽中的最小值除500。
         */
        this.resultGridDatasetResolution = null;

        /**
         * @member {number} [Ekmap.DensityKernelAnalystParameters.prototype.searchRadius]
         * @description 栅格邻域内用于计算密度的查找半径，单位与当前数据集相同。默认值为当前数据集的长宽中的最大值除30。
         */
        this.searchRadius = null;

        /**
         * @member {string} [Ekmap.DensityKernelAnalystParameters.prototype.targetDatasource]
         * @description 指定的存储结果数据集的数据源，默认为当前分析的数据集所在的数据源。
         */
        this.targetDatasource = null;

        /**
         * @member {string} Ekmap.DensityKernelAnalystParameters.prototype.resultGridName
         * @description 指定结果数据集名称。
         */
        this.resultGridName = null;

        /**
         * @member {boolean} [Ekmap.DensityKernelAnalystParameters.prototype.deleteExistResultDataset=false]
         * @description 如果用户命名的结果数据集名称与已有的数据集重名，是否删除已有的数据集。
         */
        this.deleteExistResultDataset = false;
        Util.extend(this, options);

        this.CLASS_NAME = "Ekmap.DensityKernelAnalystParameters";

    }


    /**
     * @function Ekmap.DensityKernelAnalystParameters.prototype.destroy
    ...
    /**
     * @function Ekmap.DensityKernelAnalystParameters.toObject
     * @param {Ekmap.DensityKernelAnalystParameters} densityKernelAnalystParameters -核密度分析参数类。
     * @param {Ekmap.DensityKernelAnalystParameters} tempObj - 核密度分析参数对象。
...
Ekmap.DensityKernelAnalystParameters = DensityKernelAnalystParameters;