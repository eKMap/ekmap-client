...
/**
 * @class Ekmap.RouteCalculateMeasureService
 ...
 * @example 实例化该类如下例所示：
 * (start code)
 * var parameters = new Ekmap.RouteCalculateMeasureParameters({
 *     "sourceRoute":{
 *         "type":"LINEM",
 *        "parts":[4],
 *         "points":[
 ...
 * });
 *
 * var routeCalculateMeasureService = new Ekmap.RouteCalculateMeasureService(spatialAnalystURL, {
 *     eventListeners:{
...
export class RouteCalculateMeasureService extends SpatialAnalystBase {
    ...
    constructor(url, options) {
        super(url, options);
        this.CLASS_NAME = "Ekmap.RouteCalculateMeasureService";
    }
    ...
    /**
     * @function Ekmap.RouteCalculateMeasureService.prototype.processAsync
     * @description 负责将客户端的基于路由对象计算指定点 M 值操作的参数传递到服务端。
     * @param {Ekmap.RouteCalculateMeasureParameters} params - 基于路由对象计算指定点 M 值操作的参数类。
     */
    processAsync(params) {
    ...
    /**
     * @function Ekmap.RouteCalculateMeasureService.prototype.getJsonParameters
     * @description 将参数转化为 JSON 字符串。
     * @param {Ekmap.RouteCalculateMeasureParameters} params - 基于路由对象计算指定点 M 值操作的参数类。
     * @returns {Object} 转化后的 JSON 字符串。
     */
    getJsonParameters(params) {
        var jsonParameters, jsonStr = "geometry/calculatemeasure",
            me = this;
...
Ekmap.RouteCalculateMeasureService = RouteCalculateMeasureService;