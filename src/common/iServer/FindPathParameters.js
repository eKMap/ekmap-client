...
/**
 * @class Ekmap.FindPathParameters
 ...
 * 阻抗值通过 Ekmap.TransportationAnalystParameter.weightFieldName 设置。
 * 计算最佳路径除了受阻抗影响外，还受转向字段的影响。转向值通过 {@link Ekmap.TransportationAnalystParameter.turnWeightField} 设置。
 *
 * @param {Object} options - 参数。 
 * @param {Array.<Ekmap.Geometry.Point|L.LatLng|L.Point|ol.geom.Point|number>} options.nodes - 最佳路径分析经过的结点或设施点数组。该字段至少包含两个点。
...
export class FindPathParameters {
    ...
    constructor(options) {
        ...
        /**
         * @member {boolean} [Ekmap.FindPathParameters.prototype.isAnalyzeById=false]
         * @description 是否通过节点 ID 指定路径分析的结点。
         *              指定路径分析经过的结点或设施点有两种方式：输入结点 ID 号或直接输入点坐标。
         *              当该字段为 true 时，表示通过结点 ID 指定途经点，即 Ekmap.FindPathParameters.nodes = [ID1,ID2,...]；
         *              反之表示通过结点坐标指定途经点，即 Ekmap.FindPathParameters.nodes = [{x1,y1},{x2,y2},...] 。
         */
        this.isAnalyzeById = false;

        /**
         * @member {boolean} [Ekmap.FindPathParameters.prototype.hasLeastEdgeCount=false]
        ...
        /**
         * @member {Array.<Ekmap.Geometry.Point|L.LatLng|L.Point|ol.geom.Point|number>} Ekmap.FindPathParameters.prototype.nodes
         * @description 最佳路径分析经过的结点或设施点数组，必设字段。该字段至少包含两个点。
         *              当 Ekmap.FindPathParameters.isAnalyzeById = false 时，nodes 应为点的坐标数组；
         *              当 Ekmap.FindPathParameters.isAnalyzeById = true 时，nodes 应为点的 ID 数组。
         */
        this.nodes = null;

        /**
         * @member {Ekmap.TransportationAnalystParameter} Ekmap.FindPathParameters.prototype.parameter
         * @description 交通网络分析通用参数。
         */
        this.parameter = new TransportationAnalystParameter();
        Util.extend(this, options);

        this.CLASS_NAME = "Ekmap.FindPathParameters";
    }


    /**
     * @function Ekmap.FindPathParameters.prototype.destroy
...
Ekmap.FindPathParameters = FindPathParameters;