...
/**
 * @class ol.ekmap.control.BaseLayerControl
 * @category  Control
...
var BaseLayerControl = /*@__PURE__*/ (function(Control) {
    function BaseLayerControl(opt_options) {
        var option = opt_options || {};
        this.element = document.createElement('div');
        this.target = option.target;
        if (option.element) this.element = option.element;
        var className = 'gclient-bl';
        className = className + ' ' + (option.className !== undefined ? option.className : '');
    ...
    if (Control) BaseLayerControl.__proto__ = Control;
    BaseLayerControl.prototype = Object.create(Control && Control.prototype);
    BaseLayerControl.prototype.constructor = BaseLayerControl;

    BaseLayerControl.prototype.handleRotateNorth = function handleRotateNorth() {
        this.getMap().getView().setRotation(0);
    };

    BaseLayerControl.prototype.setMap = function setMap(map) {
    ...
    /**
     * Xây dựng danh sách lớp bản đồ nền
     * @api 
     */
    BaseLayerControl.prototype.drawBaseLayer = function drawBaseLayer() {
        this.baseLayer = {};
        if (this.first) {
        ...
        if (!this.isMediumScreen_()) {
            this.btnCollapse = ekmap_element.create('div', {
        ...
        this.first = ekmap_element.create('div', {
    ...
    //  * @private
    //  */
    BaseLayerControl.prototype.getImageTile_ = function getImageTile_(layer) {
         ...
         * @private
         */
    BaseLayerControl.prototype.handleCollapseClick_ = function handleCollapseClick_(evt) {
            if (evt) {
                evt.preventDefault();
                evt.stopPropagation();
            }
            ...
            if (tog) { //expan
                ...
                for (var index in this.baseLayer) {
                    this.element.querySelectorAll('.gclient-bl-layer-' + index).forEach(function(element) {
                        ...
                        i += 5;
                        ekmap_element.setStyle(element, {
         ...
         * @private
         */
    BaseLayerControl.prototype.setActiveLayer_ = function setActiveLayer_(evt) {
            if (evt) {
                evt.preventDefault();
                evt.stopPropagation();
            }
            ...
            this.element.querySelectorAll('.gclient-bl-layer').forEach((element, index) => {
                ...
                if (id == uid) {
                    if (!hasActive) {
                        ...
                        ekmap_element.setStyle(this.first, {
            ...
            if (layerActive) layerActive.setVisible(true);
            if (!this.isMediumScreen_() && this.first) {
                var text = 'Không có nền';
                var title = 'Không có nền';
                if (layerActive) {
                    text = this.formatString(layerActive.get('name'));
                    title = layerActive.get('name');
                }
         ...
         * @private
         */
    BaseLayerControl.prototype.isMediumScreen_ = function isMediumScreen_() {
            var x = window.matchMedia("(max-width: 768px)");
         ...
         * @private
         */
    BaseLayerControl.prototype.resizeScreen_ = function resizeScreen_(evt) {
        this.drawBaseLayer();
    }
     ...
     * @private
     */
    BaseLayerControl.prototype.formatString = function formatString(str, length) {
        if (!str) return "";
